...
def write(data_path, modules):
    ...
    # Form
    st.write('# Key scenarios in board meetings')
    st.write('Thers are three reference key scenarios, you can copy and phased it or modify a little bit from them')
    st.write("""
    ### What-is?

    In 'What-is' questions, users might want to ask past information based on the fact and knowledge of financial statements. 
    Some ratios which can be calculated from the statements are also important to analyze the company's current status.

    - For example, "What was the cost of sales ratio in the last year?" 

    ### What-if: based on the fact
    In 'What-if: based on fact' questions, users might want to ask the effect to a certain account by changing another account value.
    
    - For example, "What happens to the operating income when the cost of sales increases by 10% in the last year?"

    ### What-if: forecasting

    In the 'What-if: forecasting' questions, users might want to know the future values of an account.
    ...
    if submit_button:
        ...
        error, key_information, text_warining = dialog_manager.post_process(nlu_results)
        # st.write(nlu_results)
        # st.write(key_information)
        # st.write(dialog_manager.eng2acc)
        
        if error:
            st.error(text_warining)
        else:    
            if key_information['intent'] is not None:
                ...
                st.info(f'Current Date is {dialog_manager.today.strftime("%Y-%m-%d")}')
                if text_warining is not None:
                    st.warning(text_warining)
            if key_information['intent'] is not None:
                ...
                st.write("No intent from this question...")
                with st.expander("Results from each module in json"):
                    st.write("**NLU module result: **")
                    st.write(nlu_results)
                    st.write("**Post-process result: **")
